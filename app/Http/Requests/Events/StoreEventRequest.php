...
class StoreEventRequest extends FormRequest {
  ...
  protected function prepareForValidation() {
    ...
    foreach ($this->recurrence_days as $index => $recurrenceDay) {
      $recurrenceDay['enabled'] = isset($recurrenceDay['enabled']) ? (int)$recurrenceDay['enabled'] : 0;

      $recurrenceDays[$index] = $recurrenceDay;
    }
  ...
  protected function failedValidation(Validator $validator): void {
    $this->sessionDanger('<strong>Unable to Create Event</strong><br/><br/>Please check the errors below.');
